...
class WishlistTabRow extends Component {
  state = {priorityLevel : Math.floor(Math.random() * 100) + 1 , semiCircleColor:  "red"}
  ...
  componentDidMount() {
	...
	render() {
		...
		return (
          ...
          <SemiCircleProgressBar className={classes.semiCircle} stroke={this.state.semiCircleColor} diameter={80} percentage={this.state.priorityLevel} />
          {this.state.priorityLevel >= 80 && (<WhatshotIcon style={{color: "red"}} />)}
          <Typography
            id="test"
